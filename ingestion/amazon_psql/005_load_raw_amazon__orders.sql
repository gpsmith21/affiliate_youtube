...
\set copy_command '\\copy raw_amazon_psql.orders_landing (category_name, product_name, product_asin, order_date, qty_ordered, unit_price, link_type, associate_tracking_id, indirect_sale, device_type) FROM ' :'source_path' ' WITH (FORMAT csv, HEADER, DELIMITER '','', QUOTE E''\\b'', ENCODING ''UTF8'');'
:copy_command

BEGIN; -- All-at-once transaction

-- Delete stale rows in raw_amazon_psql.orders table covering the same year but with an older refresh date
DELETE FROM raw_amazon_psql.orders
    WHERE EXTRACT(YEAR FROM raw_amazon_psql.orders.order_date::date) = :year AND refresh_date < :'refresh_date';
...
FROM raw_amazon_psql.orders
WHERE EXTRACT(YEAR FROM raw_amazon_psql.orders.order_date::date) = :year
\gset
...
INSERT INTO raw_amazon_psql.orders
...
SELECT *,
    ...
    :'refresh_date'::date,
    :'source_path'::text
FROM raw_amazon_psql.orders_landing
WHERE :'refresh_date' > :'latest_refresh_date';

TRUNCATE raw_amazon_psql.orders_landing;
