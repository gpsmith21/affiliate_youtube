...
\set copy_command '\\copy raw_amazon_psql.daily_clicks_landing (snapshot_date, daily_clicks, qty_ordered_amz, qty_ordered_other, qty_ordered_all, conversion_rate_pct) FROM ' :'source_path' ' WITH (FORMAT csv, HEADER, DELIMITER '','', QUOTE E''\\b'', ENCODING ''UTF8'');'
:copy_command

BEGIN; -- All-at-once transaction

-- Delete stale rows in raw_amazon_psql.daily_clicks table covering the same year but with an older refresh date
DELETE FROM raw_amazon_psql.daily_clicks
    WHERE EXTRACT(YEAR FROM raw_amazon_psql.daily_clicks.snapshot_date::date) = :year AND refresh_date < :'refresh_date';
...
FROM raw_amazon_psql.daily_clicks
WHERE EXTRACT(YEAR FROM raw_amazon_psql.daily_clicks.snapshot_date::date) = :year
\gset
...
INSERT INTO raw_amazon_psql.daily_clicks
...
SELECT *,
    ...
    :'refresh_date'::date,
    :'source_path'::text
FROM raw_amazon_psql.daily_clicks_landing
WHERE :'refresh_date' > :'latest_refresh_date';

TRUNCATE raw_amazon_psql.daily_clicks_landing;
